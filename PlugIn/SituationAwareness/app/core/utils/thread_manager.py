...
except Exception:  # pragma: no cover - optional direct-AirSim mode dependency
    airsim = None
...
class CameraThread(threading.Thread):
    ...
    def start_camera(self) -> bool:
        ...
        if self.use_airsim:
            if airsim is None:
                self.logger.error("Cosys-AirSim client is not available")
                return False
            try:
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    if not vision_result.detected_objects:
                        assessment = ProcessingResult(
                            timestamp=time.time(),
                            frame=vision_result.frame,
                            detected_objects=[],
                            trajectories=vision_result.trajectories or {},
                            safety_metrics=[],
                            ahp_result=None,
