...
class RoutePlanner:
    ...
    def set_spare_edge_open(self, a: str, b: str, open: bool) -> bool:
        key = self._edge_key(a, b)
        if key not in self.spare_edges:
            return False
        if open:
            if key not in self.closed_edges:
                return False
    ...
    def _build_geometry(
        ...
        *,
        ...
        if next_name and start_name in self.ports:
        ...
        if prev_name and end_name in self.ports:
...
def _demo_main() -> int:
    ...
    parser.add_argument(
        "--vertiport",
        default=None,
        help="Path to vertiport CSV (default: data/default/vertiport_default.csv)",
    )
    parser.add_argument(
        "--waypoint",
        default=None,
        help="Path to waypoint CSV (default: data/default/corridor_default.csv)",
    )
    parser.add_argument(
        "--arcs",
        action="store_true",
    ...
    vp_path = (
        Path(args.vertiport)
        if args.vertiport
        else root / "data" / "default" / "vertiport_default.csv"
    )
    wp_path = (
        Path(args.waypoint)
        if args.waypoint
        else root / "data" / "default" / "corridor_default.csv"
    )
    ...
    ax.set_xlabel("x (km)")
    ax.set_ylabel("y (km)")
    ax.set_title("Route planner demo: click start node then end node")
    ...
    def node_xy(name: str) -> Tuple[float, float]:
        if name in ports_xy:
            return ports_xy[name]
        return wps_xy[name]

    def node_is_port(name: str) -> bool:
        return name in ports_xy

    def mark_node(name: str, color: str, label: str) -> None:
        x, y = node_xy(name)
        marker_style = "o" if node_is_port(name) else "s"
        marker_size = 12 if node_is_port(name) else 10
        marker, = ax.plot(
            x,
            y,
            marker_style,
    ...
    def nearest_node(x: float, y: float, threshold: float) -> Optional[str]:
        best = None
        best_dist = threshold
        for name, (nx, ny) in planner.node_xy.items():
    ...
    def handle_click(event) -> None:
        ...
        if event.inaxes is not ax or event.xdata is None or event.ydata is None:
            return
        node_pick = nearest_node(event.xdata, event.ydata, args.threshold_km)
        port_pick = node_pick if node_pick and node_is_port(node_pick) else None
        if len(selection) < 2:
            if not node_pick:
                return
            if not selection:
                selection.append(node_pick)
                mark_node(node_pick, "tab:green", "S")
                ax.set_title(f"Start selected: {node_pick}. Click end node.")
                fig.canvas.draw_idle()
                return
            if node_pick == selection[0]:
                return
            selection.append(node_pick)
            mark_node(node_pick, "tab:red", "E")
            mode = None
            manual_path.clear()
            via_nodes.clear()
            clear_route()
        ...
        if node_pick and (node_pick in selection or port_pick):
            reset_state()
            selection.append(node_pick)
            mark_node(node_pick, "tab:green", "S")
            ax.set_title(f"Start selected: {node_pick}. Click end node.")
