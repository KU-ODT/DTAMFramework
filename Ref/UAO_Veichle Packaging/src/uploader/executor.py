"""
Mission Dispatch System — Remote Executor
...
class RemoteExecutor:
    ...
    def execute(
        ...
        try:
            ...
            # SSH 연결의 로컬 IP(Windows PC의 IP)를 가져와서 UAO_HOST로 사용
            local_ip = ssh.get_transport().sock.getsockname()[0]

            # 백그라운드 실행, PID 캡처 (UAO endpoint 환경변수 주입)
            args = f"--connection {connection_string} --grpc-port {grpc_port}"
            uao_url = f"http://{local_ip}:8000"
            cmd = (
                f"UAO_HOST={local_ip} UAO_PORT=8000 UAO_URL={uao_url} "
    ...
    def _start_mavlink_forwarder(
        ...
        """Forward PX4 localhost GCS MAVLink packets to the UAO receiver."""
        marker = f"mission_dispatch_mavlink_forwarder_{listen_port}"
        script_path = f"/tmp/{marker}.py"
        log_path = f"/tmp/{marker}.log"

        # Always refresh the forwarder on each SITL launch.  The UAO host can
