...
const app = createApp({
  data() {
    return {
      ...
      runtimeStatus: {
        status: 'unknown',
        registeredAircraft: 0,
        pendingMissions: 0,
        missionStatusTotal: 0,
        missionRxCount: 0,
        timeRxCount: 0,
        lastMissionAt: null,
        lastMissionSource: null,
        lastMissionPayload: null,
        lastMissionResponse: null,
        lastTimeAt: null,
        lastTimePayload: null,
        lastTimeSnapshot: null,
      },
      simulationTime: {
        time: null,
        seconds: null,
  ...
  mounted() {
    ...
    this.runtimePollTimer = window.setInterval(() => this.fetchRuntimeStatus(), 700);
  ...
  methods: {
    ...
    runtimeReady() {
      return this.runtimeStatus?.status === 'ok';
    },
    runtimeMissionLabel() {
      const response = this.runtimeStatus?.lastMissionResponse;
      const payload = this.runtimeStatus?.lastMissionPayload;
      const flightPlan = response?.flightPlanNumber || payload?.flightPlanNumber || '--';
      const aircraft = response?.aircraftId || payload?.aircraftId || '--';
      const httpStatus = response?.httpStatus ? `HTTP ${response.httpStatus}` : 'no response';
      return `FP${flightPlan} / ${aircraft} / ${httpStatus}`;
    },
    runtimeTimeLabel() {
      const snapshot = this.runtimeStatus?.lastTimeSnapshot || this.runtimeStatus?.time || {};
      return snapshot.time || this.simulationTime.time || '--:--:--';
    },
    ...
    async fetchRuntimeStatus() {
      ...
      try {
        const res = await fetch('/api/v1/runtime/status', { cache: 'no-store' });
        if (!res.ok) return;
        const data = await res.json();
        this.runtimeStatus = {
          ...this.runtimeStatus,
