"""Run the DTAMAirVehicle web app.
...
# DTAMAirVehicle 패키지 import 용 + DTAM_SDK 번들 import 용
...
from DTAMAirVehicle.backend.app import create_app  # noqa: E402
from DTAMAirVehicle.service.integrated_service import (  # noqa: E402
    ClockMode,
...
def open_browser(url: str, fullscreen: bool = True) -> None:
    ...
    if browser_path:
        ...
        return
    print("[DTAMAirVehicle] dedicated browser not found; using default webbrowser.open")
    webbrowser.open(url)
...
def main() -> None:
    ...
    if args.port != requested_port:
        print(f"[DTAMAirVehicle] port {requested_port} is busy; using {args.port}.")

    url = f"http://{args.host}:{args.port}"
    print(f"[DTAMAirVehicle] GUI : {url}")
    print(f"[DTAMAirVehicle] root: {ROOT}")
    print(f"[DTAMAirVehicle] DTAM target: {args.target_ip}:{args.target_port}  my_port={args.my_port}")
