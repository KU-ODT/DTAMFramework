(() => {
  ...
  function renderStatus(status) {
    publisherStatus.textContent =
      `target=${status.target_ip}:${status.target_port}  local=${status.my_port}  `
      + `connected=${yesNo(status.publisher_connected)}`
    ...
    const capture = status.keyboard_capture || {};
    if (capture.active) {
      setKeyboardCapture(true, capture.source || "windows-global");
      if (capture.source === "windows-global" && manual.input) {
        renderAxes({
    ...
    } else if (keyboardCaptureSource === "windows-global") {
    ...
    controlStatus.textContent =
      ...
      + `capture=${capture.active ? (capture.source || "unknown") : "off"}`
      + (capture.last_error ? `  capture_error=${capture.last_error}` : "");
    ...
    activateKeyboard.classList.toggle("is-active", keyboardCapture);
  ...
  function setKeyboardCapture(enabled, source = "") {
    keyboardCapture = enabled;
    keyboardCaptureSource = enabled ? (source || "browser") : "";
    if (!enabled) {
      pressed.clear();
    }
    activateKeyboard.classList.toggle("is-active", enabled);
    if (!enabled || keyboardCaptureSource === "browser") {
  ...
  activateKeyboard.onclick = async () => {
    try {
      ...
      if (capture.active) {
        ...
        setKeyboardCapture(true, "browser");
  ...
  releaseKeyboard.onclick = async () => {
    try {
      const result = await api("/api/control/keyboard/stop", { method: "POST" });
      applyResult(result);
    } catch {
      try {
  ...
  window.addEventListener("keydown", (event) => {
    if (!keyboardCapture || keyboardCaptureSource !== "browser") {
      return;
    }
  ...
  async function sendManualInput() {
    ...
    if (keyboardCaptureSource !== "windows-global") {
      renderAxes(input);
    }
    if (!keyboardCapture || keyboardCaptureSource !== "browser") {
      return;
