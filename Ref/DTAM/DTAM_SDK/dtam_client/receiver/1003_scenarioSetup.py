"""Scenario Setup (MSG 1003) 수신/파싱기."""
...
from dtam_client.schema.msg_1003 import validate_message
...
class ReceiveResult:
    ...
    operation_time: Optional[Dict[str, str]] = None
    vertiports: List[Dict[str, Any]] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "totalAircraftCount": self.total_aircraft_count,
            "mainVehicleType": self.main_vehicle_type,
            "errors": self.errors,
        }
...
def parse(data: Union[bytes, str, Dict[str, Any]]) -> ReceiveResult:
    ...
    try:
        ...
        result.operation_time = dict(obj.get("operationTime", {}))
        result.vertiports = list(obj.get("vertiports", []))
        rn = obj.get("routeNetwork", {})
        result.waypoints = list(rn.get("waypoints", []))
        result.total_aircraft_count = obj.get("totalAircraftCount")
        result.main_vehicle_type = obj.get("mainVehicleType")
        return result
