...
class DemTileProvider:
    ...
    def __post_init__(self) -> None:
        ...
        for tif in sorted(self.dem_dir.glob("*.tif")):
            ...
            self._nodata_values.append(dataset.nodata)
    ...
    def sample_elevation(self, lon: float, lat: float) -> Optional[float]:
        if not self.available or rasterio is None:
            return None
        try:
        ...
        with self._lock:
            for vrt, nodata in zip(self._vrts, self._nodata_values):
                if x < vrt.bounds.left or x > vrt.bounds.right or y < vrt.bounds.bottom or y > vrt.bounds.top:
                    continue
                try:
                ...
                if not np.isfinite(value):
                    continue
                if nodata is not None and math.isclose(value, float(nodata), abs_tol=1e-6):
                    continue
                if value <= -32000:
                    continue
