...
| **V-5** | **다회 운항(시간표) 지원** ★S2 전제 | `integrated_service.py` — `_on_scheduled_flight` 의 plan 교체 모델 (`기존 계획이 있으면 교체`) → **기체당 plan 큐** 로 확장 | 현재 같은 aircraftId 의 새 3001 이 오면 기존 plan 을 폐기 — S2 셋 (3,360편/136대, 기체당 ~25편) 이 기체당 1편으로 붕괴됨 (실측: 세션 22, 비행 2대). 판별 규칙: 같은 aircraftId 의 새 3001 이 **같은 fpn + 높은 planVersion** 이면 교체(재계획 — 기존 동작 유지), **다른 fpn** 이면 plan 큐에 추가(다음 편). 편 종료 (착륙·turnaround) 후 같은 기체의 다음 std 편을 자동 시작. 포맷 비호환 아님 — 3,360편 전부 SDK 검증 통과, 운용 모델(1:1→1:N)의 간극. **임시 우회**: 스케줄러에서 기체당 1편 셋 (예: 136편/136대) 을 export 하면 현 구조로도 전 기체 비행 가능 |
