...
| **V-2** | **데모 바람 자체 생성** | Vehicle 내부 — 자체 WindModel / 바람 데이터 생성 (트리거: 1002 `wind.grade=serious` 또는 2002 `scenarioId="S2"`, 구현 선택) | Vehicle 은 **자체적으로 바람을 생성·적용** (사용자 결정). 트리거는 운영자가 기상 패널에서 바람 등급 serious 선택 시 발행되는 1002 (기존 기능). **5004 는 보류 (발행처 없음)** — Vehicle 자체 바람 생성으로 대체. 참고: 현재 `config.wind_enabled=False` 로 부팅 |
...
| **P-3** | **S2: 궤적 외삽 + 3003 setSpeed 발행** | 내부 로직 + `self.send(parse_payload("3003", {...}))` | 4001 스트림만으로 90s lookahead 외삽 (5004 의존 없음 — 보류) → 수평 분리 <300m 수렴 예측 → **3003**: `actions=[{type:"setSpeed", targetSpeed:40.0}]`, `reasonCode="LOSS_OF_SEPARATION_RISK"`, `commandId="TMP-PSU-{aircraftId}-{YYYYMMDD}-{seq}"`, **`scenarioId:"S2"` 동봉** (Vehicle 세부 세팅 분기용). 분리 ≥300m 회복 후 (선택) setSpeed 복원 또는 rejoinPlan(atSeq) |
