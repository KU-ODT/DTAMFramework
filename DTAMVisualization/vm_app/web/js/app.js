...
(function () {
  ...
  function renderSnapshot(snapshot) {
    ...
    setValIfBlurred("camera-hz", stream.camera_hz ?? 5);
    setValIfBlurred("camera-quality", stream.camera_quality ?? 80);
    ...
    els["btn-streaming-apply"]?.addEventListener("click", async () => {
      await api("/api/streaming", {
        ...
        body: JSON.stringify({
          ...
          camera_hz: Number(els["camera-hz"]?.value || 5),
          camera_quality: Number(els["camera-quality"]?.value || 80),
