...
_MAX_RECENT = 200
_COMPACT_PAYLOAD_MIDS = {"4001", "4101", "5001"}
...
class MessageCounter:
    ...
    def to_dict(self) -> Dict[str, Any]:
        compact = self.mid in _COMPACT_PAYLOAD_MIDS
        return {
            "mid": self.mid,
            "name": self.name,
            ...
            "last_rx_payload": _payload_preview(self.mid, self.last_rx_payload) if compact else self.last_rx_payload,
            "last_tx_payload": _payload_preview(self.mid, self.last_tx_payload) if compact else self.last_tx_payload,
            "payload_compacted": compact,
            "hz": self.hz,
        }
...
def _payload_preview(mid: str, payload: Any) -> Any:
    ...
    if mid == "4001":
        vehicle_ids = [
            ...
            if key not in ("timestamp", "vehicles") and isinstance(value, dict)
        ]
        if isinstance(payload.get("vehicles"), dict):
            vehicle_ids.extend(str(key) for key in payload["vehicles"].keys())
        return {
    ...
    if mid == "5001":
        keys = (
            "timestamp", "aircraftId", "vehicleId", "roll", "pitch", "yaw",
            "throttle", "brake", "mode",
        )
        preview = {key: payload.get(key) for key in keys if key in payload}
        return preview or {"keys": list(payload.keys())[:12], "key_count": len(payload)}
    return payload
