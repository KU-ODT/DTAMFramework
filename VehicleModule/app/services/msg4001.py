...
def _normalized_energy(energy: Optional[Dict[str, Any]], battery_pct: float) -> Dict[str, Any]:
    """Return a 4001 energy block tied to the authoritative cumulative battery.

    Some upstream/high-fidelity providers expose instantaneous "energy" or
    estimated SOC values that can rise again when the aircraft slows down or
    descends.  DTAM 4001 should publish remaining battery, so keep any extra
    provider fields but always pin the public battery/SOC fields to the
    monotonic value tracked by VehicleModule.
    """
    normalized: Dict[str, Any] = deepcopy(energy) if isinstance(energy, dict) else {}
    batt = float(_clamp(float(battery_pct), 0.0, 100.0))
    normalized["battery_pct"] = batt
    normalized["state_of_charge_pct"] = batt
    return normalized
...
def _estimate_motor_rpm(speed_mps: float, climb_mps: float) -> Tuple[float, float, float, float]:
    ...
    순서는 QuadX 기준: 1 우측 앞, 2 좌측 뒤, 3 좌측 앞, 4 우측 뒤.
    """
    base = 2000.0
...
def build_vehicle_payload(
    *,
    ...
    batt = _clamp(float(battery_pct) if battery_pct is not None else float(context.battery_pct), 0.0, 100.0)
    ...
    energy_payload = _normalized_energy(energy, batt)

    payload = {
        "currentWaypointId": str(waypoint_id),
        # Operation/GUI monitor? attitude.yaw(??? pose-frame yaw)?
        # ??? ????? ??? ?? ?? heading/speed? top-level?? ??.
        "speedMps": float(_clamp(float(speed_mps), 0.0, 1000.0)),
        "groundSpeedMps": float(_clamp(math.hypot(float(vn), float(ve)), 0.0, 1000.0)),
        "headingDeg": float(float(heading_deg) % 360.0),
        "trackHeadingDeg": float(float(track_deg) % 360.0),
        "position": {
            "north": float(north_m),
            "east": float(east_m),
            "down": float(down_m),
    ...
    if isinstance(navigation, dict) and navigation:
        payload["navigation"] = deepcopy(navigation)
    return payload
