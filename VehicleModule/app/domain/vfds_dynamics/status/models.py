...
class VelocityNedData(BaseModel):
    """NED velocity in m/s."""
    north: float = 0.0
    east: float = 0.0
    down: float = 0.0
...
class AircraftStatus(BaseModel):
    ...
    velocityNed: VelocityNedData = Field(default_factory=VelocityNedData)
    actuator: ActuatorData = Field(default_factory=ActuatorData)
    motorRpm: List[float] = Field(default_factory=lambda: [0.0, 0.0, 0.0, 0.0])

    # Optional provider telemetry used only by ICD conversion helpers.  They are
    # excluded from legacy AircraftStatus serialization so existing consumers do
    # not see a contract change.
    speedMps: Optional[float] = Field(default=None, exclude=True)
    energy: Dict[str, Any] = Field(default_factory=dict, exclude=True)
    batteryPct: Optional[float] = Field(default=None, exclude=True)
    battery_pct: Optional[float] = Field(default=None, exclude=True)
    stateOfChargePct: Optional[float] = Field(default=None, exclude=True)
    state_of_charge_pct: Optional[float] = Field(default=None, exclude=True)
