...
async def broadcast_status(status: AircraftStatus):
    # Keep the existing VFDS live WebSocket payload exactly as AircraftStatus
    # JSON, and additionally feed the optional raw 4001 debug WebSocket.
...
async def dtam_websocket_endpoint(websocket: WebSocket):
    """Auxiliary raw 4001 WebSocket for ICD inspection.

    This is intentionally not a replacement for StateServerModule's standard
    DTAM SDK `/ws/dtam` envelope.  VehicleModule remains the authoritative 4001
    publisher in the normal DTAM flow.
    """

    client = getattr(websocket, "client", None)
    await dtam_manager.connect(websocket)
    logger.info("DTAM raw 4001 WebSocket connected: %s", client)
    ...
    try:
        ...
        logger.info("DTAM raw 4001 WebSocket disconnected: %s", client)
