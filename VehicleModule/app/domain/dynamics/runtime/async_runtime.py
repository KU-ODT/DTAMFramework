...
streaming FlightTrajectoryPoint records at a configurable rate (default 30 Hz).
...
class WallClockSource(ClockSource):
    ...
    def __init__(self, tick_interval: float = DEFAULT_SIMULATION_TICK_S) -> None:
...
class FreeRunClockSource(ClockSource):
    ...
    async def wait_next_tick(self) -> float:
        self._t += DEFAULT_SIMULATION_TICK_S  # nominal
