...
class Msg2002_DtamExecute:
    ...
    Vehicle 은 이 값으로 시나리오별 동작을 분기한다.
    - "S1": 정상 운항 (분기 없음)
    - "S2": PSU 속도 조정 재계획 시나리오 — Vehicle 측 특별 무장 없음
      (1002 wind.weather 바람 적용과 3003 setSpeed 수행은 표준 메시지 처리로 동작)
    - "S3": 배터리 열화 프로필 활성화 + UAO 겸업 판단 로직 무장(arm)
