...
|---|---|---:|---:|---:|---|
| `motor_rpm` | list\<float\> | rpm | 4 | 0 ~ 6000 | 모터 RPM QuadX 순서: 1 우측 앞, 2 좌측 뒤, 3 좌측 앞, 4 우측 뒤 |
...
| 필드 | 타입 | 단위 | 범위 | 설명 |
|---|---|---:|---:|---|
| `battery_pct` | float | % | 0 ~ 100 | 남은 배터리 잔량 |
| `state_of_charge_pct` | float | % | 0 ~ 100 | SOC 별칭. `battery_pct`와 동일 값 |

### 2.9 collision (선택)

일반 비행 중에는 생략된다. 4103 `Vehicle Collision Event` 수신 후
VehicleModule이 hold/emergency/abort 반응을 적용 중일 때 각 비행체
payload에 추가된다.

| 필드 | 타입 | 설명 |
|---|---|---|
| `active` | bool | 충돌 반응으로 비행체를 hold 중이면 `true` |
| `lastEventId` | str | 반응에 사용된 최근 4103 이벤트 ID |
| `objectName` | str | AirSim/Unreal이 보고한 충돌 대상 |
| `severity` | str | `info`, `warning`, `critical`, `fatal` |
| `recommendedAction` | str | 4103에서 요청한 조치 |
| `responseMode` | str | VehicleModule 반응: `hold`, `emergency_stop`, `abort` |
| `timestamp` | str | 4103 이벤트 시각 |
...
`navigation`? VehicleModule? ?? MSG 3001 ????? ???? ???? ?? ?? ?? ????. ???/HUD ?????, Keyboard/Joystick ?? ???? ? ??? ????? Autopilot ??? ????? ???.

| ?? | ?? | ?? | ?? |
|---|---|---:|---|
| `flightPlanNumber` | int | - | ?? ??? ??? MSG 3001 ???? ?? |
| `currentWaypointId` | str | - | ??? payload? ?? ??? ?? waypoint ID |
| `nextWaypointId` | str | - | ?? ?? ?? waypoint, ?? `{flightPlanNumber}-{seq}` |
| `targetLLA` | object | - | HUD ?? ??? ???? WGS84 ??? |
| `targetLLA.lat` | float | deg | ?? ?? |
| `targetLLA.lon` | float | deg | ?? ?? |
| `targetLLA.alt` | float | m | ?? ?? |
| `bearingToTargetDeg` | float | deg | ?? GPS ???? `targetLLA`??? ?? ?? ??? `[0, 360)` |
| `distanceToTargetM` | float | m | ????? ?? ?? |
| `verticalDeltaM` | float | m | `targetLLA.alt - ?? ??` |
| `source` | str | - | ?? ?? ??, ?: `3001`, `manual-route` |
...
Unreal? `bearingToTargetDeg`? ??? I ? HUD? ?? heading ruler? ??? ?? ?? ??? ????. ?? ??? ?? ??? ???? ?/? ?? ???? ???? ??? ????.
