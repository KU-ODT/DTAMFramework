...
| `scenarioId` | str (optional) | 데모 시나리오 식별 — Vehicle 의 시나리오별 분기용 (`"S1"`\|`"S2"`\|`"S3"`). S3: 배터리 열화 프로필 + UAO 겸업 판단 무장, S2: 바람 영향 수용. 생략 시 일반 실행 | `S3` |
...
- `scenarioId` 는 Operation Console 이 실행 시점에 현재 데모 시나리오(S1/S2/S3)를 Vehicle 에 알리는 용도이다.
  Vehicle 은 `on_dtam_execute()` 에서 `scenarioId` 로 분기: S1 → 정상 실행, S2 → 바람 영향 수용 모드, S3 → 배터리 열화 프로필 활성화 + UAO 겸업 판단 로직 무장 (UAO 역할은 별도 모듈이 아닌 VehicleModule 이 겸업).
