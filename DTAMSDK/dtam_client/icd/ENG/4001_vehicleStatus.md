...
|---|---|---:|---:|---:|---|
| `motor_rpm` | list\<float\> | rpm | 4 | 0 ~ 6000 | Motor RPM values in QuadX order: 1 front-right, 2 rear-left, 3 front-left, 4 rear-right |
...
| Field | Type | Unit | Range | Description |
|---|---|---:|---:|---|
| `battery_pct` | float | % | 0 ~ 100 | Remaining battery |
| `state_of_charge_pct` | float | % | 0 ~ 100 | SOC alias, same value as `battery_pct` |

### 2.9 collision (optional)

`collision` is omitted during normal flight. VehicleModule adds it after a
4103 `Vehicle Collision Event` when a hold/emergency/abort response is active.

| Field | Type | Description |
|---|---|---|
| `active` | bool | `true` while the collision response is holding the vehicle |
| `lastEventId` | str | Last 4103 event id used for this response |
| `objectName` | str | Object reported by AirSim/Unreal |
| `severity` | str | `info`, `warning`, `critical`, or `fatal` |
| `recommendedAction` | str | Action requested by the 4103 event |
| `responseMode` | str | VehicleModule response: `hold`, `emergency_stop`, or `abort` |
| `timestamp` | str | 4103 event timestamp |
...
"collision": {
  "active": true,
  "lastEventId": "COL-UAM0001-000001",
...
### 2.10 navigation (optional)

`navigation` is an optional route-relative guidance object generated by VehicleModule from the active MSG 3001 flight plan. It is intended for visualization/HUD guidance only; Keyboard and Joystick vehicles do not become autopilot-controlled because this field is present.

| Field | Type | Unit | Description |
|---|---|---:|---|
| `flightPlanNumber` | int | - | MSG 3001 flight plan number used to compute guidance |
| `currentWaypointId` | str | - | Current waypoint id already reported by the vehicle payload |
| `nextWaypointId` | str | - | Target waypoint currently used for guidance, usually `{flightPlanNumber}-{seq}` |
| `targetLLA` | object | - | WGS84 target point for the guidance marker |
| `targetLLA.lat` | float | deg | Target latitude |
| `targetLLA.lon` | float | deg | Target longitude |
| `targetLLA.alt` | float | m | Target altitude |
| `bearingToTargetDeg` | float | deg | True bearing from current GPS position to `targetLLA` in `[0, 360)` |
| `distanceToTargetM` | float | m | Horizontal distance to the guidance target |
| `verticalDeltaM` | float | m | `targetLLA.alt - current_altitude` |
| `source` | str | - | Guidance source, e.g. `3001` or `manual-route` |
...
"navigation": {
  ...
  "currentWaypointId": "1201-4",
  "nextWaypointId": "1201-5",
  "targetLLA": { "lat": 37.581542, "lon": 127.048947, "alt": 304.8 },
  "bearingToTargetDeg": 73.4,
  "distanceToTargetM": 1420.5,
  "verticalDeltaM": 82.1,
  "source": "3001"
}
```

Unreal uses `bearingToTargetDeg` to draw the orange required-direction marker on the I-key HUD heading ruler. If the bearing is outside the visible ruler span, the marker is clamped to the left/right edge with an arrow.
