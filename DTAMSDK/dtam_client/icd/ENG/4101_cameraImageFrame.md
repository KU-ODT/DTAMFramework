...
MSG 4101 transfers camera image frames from Visualization to the server and Operations Console and Situation Awareness plug-in. It is separate from MSG 4001 because images are large while 4001 must remain a small vehicle-status message.
...
- `DTAMSDK.dtam_client._ws_client.DtamWsClient.send(..., image_bytes=...)` converts `image_bytes` to `image_b64`.
- `StateServerModule` decodes `image_b64`, stores the latest frame, and forwards MSG 4101 through the server to the `monitoring` and `situation_awareness` roles.
- SDK receivers mirror the envelope-level `image_b64` into the parsed `Msg4101_CameraImageFrame.image_b64` field for plug-ins that need direct image access.
