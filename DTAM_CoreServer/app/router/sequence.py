...
GET /docs/sequence — Phase별 메시지 흐름 시퀀스 다이어그램 (SVG)
"""
...
.seq-group{position:absolute;left:0;display:flex;align-items:center}
.seq-group-label{background:#1f6feb33;color:#58a6ff;padding:4px 14px;border-radius:6px;font-size:13px;font-weight:700;border:1px solid #1f6feb55}
...
async function render(){
  ...
  const colW=180,rowH=48,groupH=38,totalW=colW*ACTORS.length;
  ...
  for(const m of MSGS){
    ...
    const proto=m.proto==="TCP"?"tcp":m.proto==="UDP"?"udp":"int";
    const sc=proto==="tcp"?"#3b78d8":proto==="udp"?"#43a047":"#8892a6";
  ...
  html+=`<div class="seq-body" style="width:${totalW}px;height:${bodyH}px;position:relative">${svg}${labels}${groups}</div>`;
  html+=actorBar(false);
  el.innerHTML=html;
  const ssg=document.getElementById("ssg");
  const gm=[];let gy=0;
  for(const m of MSGS){if(m.group){gm.push({y:gy,label:m.group});gy+=groupH}else gy+=rowH}
  const sync=()=>{let c=gm[0]?.label||"";for(const g of gm)if(g.y<=el.scrollTop)c=g.label;ssg.textContent=c};
