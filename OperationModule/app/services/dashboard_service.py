...
from app.schemas.module import ActionItem, MetricItem, ModuleCard, ModuleOverview
...
MODULES: list[ModuleCard] = [
    ModuleCard(
        ...
        subtitle="Mission flow and execution",
        description="Control mission mode, operator flow, playback context, and live execution state from one workspace.",
        icon="mission",
        accent="#53e0c7",
        endpoint="/api/v1/mission/overview",
    ),
    ModuleCard(
        slug="dtam-extensions",
        title="DTAM Extension Modules",
        subtitle="Stakeholders",
        description="Reserved workspace for DTAM stakeholder-facing extension modules.",
        icon="dtam-extensions",
        accent="#78c8ff",
    ...
    ModuleCard(
        slug="plugin",
        title="DTAM Plugin",
        subtitle="Extensible runtime slots",
        description="Manage operator plug-in slots and prepare future runtime launch points for external instances.",
        icon="plugin",
        accent="#8db8ff",
        endpoint="/api/v1/system/overview",
    ),
    ModuleCard(
        ...
        subtitle="Server logs and data feeds",
        description="Open the DTAM server console directly for data feeds, logs, and server-side inspection.",
        icon="data",
        accent="#b8e86d",
...
MODULE_OVERVIEWS: dict[str, ModuleOverview] = {
    ...
    "plugin": ModuleOverview(
        slug="plugin",
        headline="Plug-In",
        summary="Reserve horizontally scalable runtime slots for operator plug-ins such as PSU, VFDS, and VPO. The first release keeps the shell ready and leaves runtime launch hooks for later integration.",
        status="Plug-in shell ready",
        metrics=[
            MetricItem(label="Runtime Slots", value="04", hint="Named plug-in boxes visible to the operator"),
            MetricItem(label="Launch Hooks", value="Pending", hint="External instance startup will be wired later"),
            MetricItem(label="Selection Model", value="Single", hint="One active plug-in target per run action"),
        ],
        actions=[
            ActionItem(id="psu-slot", label="PSU", description="Reserved slot for the PSU plug-in runtime."),
            ActionItem(id="vfds-slot", label="VFDS", description="Reserved slot for the VFDS plug-in runtime."),
            ActionItem(id="vpo-slot", label="VPO", description="Reserved slot for the VPO plug-in runtime."),
        ],
    ),
