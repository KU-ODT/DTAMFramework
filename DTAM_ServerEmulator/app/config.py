...
- DB 루트 디렉토리 (기본: DTAMFramework/DB/)
- HTTP GUI 포트
...
ROOT_DIR = Path(__file__).resolve().parents[1]          # DTAM_ServerEmulator/
...
# 메시지 ID → DB 저장 폴더명. (timestamp 하위 경로)
...
def default_modules() -> List[ModuleEndpoint]:
    ...
    return [
        ModuleEndpoint(
            ...
            expected_source="DTAM_MissionPlanner",
        ),
        ModuleEndpoint(
        ...
        ModuleEndpoint(
            ...
            expected_source="DTAMAirMobility",
        ),
        ModuleEndpoint(
