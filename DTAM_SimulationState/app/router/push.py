...
from DTAM_CoreServer.app.schema.icd_registry import (
    ICD_REGISTRY,
    get_example_payload,
    to_dict,
)
...
def _create_push_handler(msg_id: str):
    ...
    async def handler(request: Request, body: Any = Body(None)) -> JSONResponse:
        ...
        # [NEW] 1002 명령인 경우 내부 엔진에 즉시 전달 (브로드캐스트 전)
        if msg_id == "1002":
            try:
                from DTAM_CoreServer.app.model.traffic import TrafficEvent
                import time
                fake_evt = TrafficEvent(
                    ts=time.time(), mid=msg_id, kind="rx",
                    full_payload=payload_dict
                )
                if hub.on_event:
                    hub.on_event(fake_evt)
            except Exception:
                pass
        ...
        if role:
            ...
            results = {}
            sent_count = 0
            for f_role in forward_roles:
                res = hub.push_to_role(f_role, msg_id, payload_dict)
                results[f_role] = res
                if res.get("ok"):
                    sent_count += 1
            result = {
