"""Run the VPOModule Vertiport Monitoring web app."""
...
except Exception:  # pragma: no cover - fallback for standalone preview
    import socket
...
from ExtenstionModule.VPOModule.app.server import create_app  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DTAM Vertiport Operations Monitoring")
    parser.add_argument("--host", default="127.0.0.1", help="Host address to bind.")
    parser.add_argument("--port", type=int, default=8110, help="GUI/API port.")
...
def main() -> None:
    ...
    if args.port != requested_port:
        print(f"[VPOModule] port {requested_port} is busy; using {args.port}.")

    url = f"http://{args.host}:{args.port}"
    print(f"[VPOModule] GUI  : {url}")
    print(f"[VPOModule] root : {ROOT}")
