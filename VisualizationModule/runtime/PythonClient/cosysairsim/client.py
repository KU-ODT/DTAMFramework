...
class VehicleClient:
    ...
    def simIsWeatherEnabled(self):
        """
        Check whether weather effects are currently enabled.

        Returns:
            bool: True if enabled, False otherwise
        """
        return self.client.call('simIsWeatherEnabled')

    def simGetWeatherParameter(self, param):
        """
        Get the current value of a weather parameter.

        Args:
            param (WeatherParameter): Weather effect to query

        Returns:
            float: Current intensity of the effect, Range 0-1
        """
        return self.client.call('simGetWeatherParameter', param)
    ...
    def simGetCameraFrame(self, camera_name, image_type, vehicle_name='', annotation_name="", quality=80):
        """
        Get the latest available camera frame as a JPEG produced directly from the Unreal render target.

        Args:
            camera_name (str): Name of the camera. Named cameras such as "front_center" are preferred.
            image_type (ImageType): Type of image required.
            vehicle_name (str, optional): Name of the vehicle with the camera.
            annotation_name (str, optional): Name of the annotation to be applied if using image type Annotation.
            quality (int, optional): JPEG quality from 1 to 100.
    ...
    def simGetVpoCameraFrame(self, camera_id, quality=70):
        """
        Get an on-demand VPO vertiport CCTV frame as JPEG from an Unreal SceneCapture2D.

        Args:
            camera_id (str): VPO camera id from vpo_camera_views.csv, for example "VPO-001-01".
            quality (int, optional): JPEG quality from 10 to 95.

        Returns:
            bytes: Binary string literal of compressed jpeg image, or None if unavailable.
        """
        result = self.client.call('simGetVpoCameraFrame', str(camera_id), int(quality))
        if result == "" or result == "\0":
            return None
        return result
    ...
    def simSetVehicleVisualState(
        ...
        """
        Set vehicle-only visual state directly in Unreal.

        This supplements `simSetVehiclePose` for parts that AirSim pose APIs do not cover:
        tilt rotors, coupled control surfaces, and per-rotor visual RPM.

        Args:
            tilt_left_ratio (float): Left tilt ratio from MSG 4001 actuator.tilt_left.
            tilt_right_ratio (float): Right tilt ratio from MSG 4001 actuator.tilt_right.
            aileron_deg (float): Coupled aileron command/state in degrees.
            rudder_left_deg (float): Left rudder command/state in degrees.
            rudder_right_deg (float): Right rudder command/state in degrees.
            motor_rpm (list[float]): Four-channel motor RPM from MSG 4001 propulsion.motor_rpm.
                QuadX order: 1 front-right, 2 rear-left, 3 front-left, 4 rear-right.
            vehicle_name (str, optional): Name of the vehicle to update.

        Returns:
            bool: True if the visual state was applied.
        """
        return self.client.call(
            'simSetVehicleVisualState',
            float(tilt_left_ratio),
            float(tilt_right_ratio),
            float(aileron_deg),
            float(rudder_left_deg),
            float(rudder_right_deg),
            [float(item) for item in motor_rpm],
            vehicle_name,
        )

    def simSetVehicleTelemetry(
        self,
        vehicle_id,
        current_waypoint_id,
        position_north_m,
        ...
        """
        Forward DTAM MSG 4001 telemetry to Unreal HUD/monitoring widgets.

        `simSetVehiclePose` updates the pawn transform, but AirSim internal
        kinematics can stay near zero when the pose is externally injected.
        This RPC lets the I-key HUD display the authoritative DTAM velocity,
        GPS, barometer, waypoint and battery values from MSG 4001.
        """
        args = [
            vehicle_id,
            current_waypoint_id,
        ...
        if battery_fraction is not None:
            args.append(float(battery_fraction))
        return self.client.call('simSetVehicleTelemetry', *args)

    def simSetVehicleNavigationTarget(
        self,
        vehicle_id,
        ...
        """
        Forward DTAM MSG 4001 optional navigation guidance to the Unreal I-key HUD.

        This does not control the aircraft.  It only tells the HUD where the
        mission/manual-route target bearing is, so Keyboard/Joystick vehicles can
        still show route-relative guidance while staying under manual control.
        """
        return self.client.call(
            'simSetVehicleNavigationTarget',
            vehicle_id,
            bool(has_navigation_target),
    ...
    def simAdjustPrimaryView(self, yaw_delta_deg, zoom_input=0.0, vehicle_name=''):
        """
        Adjust the Unreal main view camera director directly.

        This targets the active CameraDirector view shown in the Visualization app,
        not the AirSim sensor camera image stream.

        Args:
            yaw_delta_deg (float): Horizontal orbit / look delta in degrees.
            zoom_input (float, optional): Mouse-wheel style zoom input. Positive zooms in.
            vehicle_name (str, optional): Vehicle to retarget the chase camera to before applying input.

        Returns:
            bool: True if the main view camera was adjusted.
        """
        return self.client.call(
            'simAdjustPrimaryView',
            float(yaw_delta_deg),
            float(zoom_input),
            vehicle_name,
        )

    def simGetPrimaryViewVehicle(self):
        """
        Return the AirSim vehicle name currently selected in the Unreal main view.

        DTAM uses this to route Keyboard/Joystick authority to the vehicle that
        the operator is actually watching via the 1/2/3/4/5 view keys.
        """
        return self.client.call('simGetPrimaryViewVehicle')
    ...
    def simSpawnAbnormalObstacle(
        ...
        """
        Spawns/updates/removes a DTAM abnormal situation obstacle in Unreal.

        This DTAM extension is used by 5003 Abnormal Situation Command.  The
        first supported abnormal_type is ``bird_flock``.
        """
        return self.client.call(
            'simSpawnAbnormalObstacle',
            command_id,
            abnormal_type,
            action,
            float(lat),
