...
namespace airlib
{
    ...
    public: //fields
        ...
        int initial_view_mode = 4; //ECameraDirectorMode::CAMERA_DIRECTOR_MODE_SPRINGARM_CHASE
    ...
    private:
                ...
                if (has_default_settings) {
                    ...
                    if (settings_version_actual == 1) {
                        const std::vector<std::string> all_changed_keys = {
                            "AdditionalCameras", "CaptureSettings", "NoiseSettings", "UsageScenario", "SimpleFlight"
                        };
        ...
        void loadViewModeSettings(const Settings& settings_json)
        {
            ...
            if (view_mode_string == "") {
                if (simmode_name == kSimModeTypeComputerVision)
                                                                    ...
                                                                    const CameraSetting& camera_defaults)
        {
            ...
            std::unique_ptr<VehicleSetting> vehicle_setting(new VehicleSetting());
            if (vehicle_type == kVehicleTypeSimpleFlight) {
                // TODO: select remote if available else keyboard. Keyboard is not supported yet.
