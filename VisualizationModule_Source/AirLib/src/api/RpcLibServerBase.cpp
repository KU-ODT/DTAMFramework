...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simIsWeatherEnabled", [&]() -> bool {
            return getWorldSimApi()->isWeatherEnabled();
        });

        pimpl_->server.bind("simGetWeatherParameter", [&](WorldSimApiBase::WeatherParameter param) -> float {
            return getWorldSimApi()->getWeatherParameter(param);
        });
        ...
        pimpl_->server.bind("simGetCameraFrame", [&](const std::string& camera_name, ImageCaptureBase::ImageType type, const std::string& vehicle_name, const std::string& annotation_name, int quality) -> vector<uint8_t> {
            return getWorldSimApi()->getCameraFrame(type, CameraDetails(camera_name, vehicle_name), annotation_name, quality);
        });
