...
void PawnSimApi::setupLightsFromSettings(const common_utils::UniqueValueMap<std::string, ALight*>& lights)
{
    //add lights that already exists in pawn
    lights_.clear();
    for (const auto& p : lights.getMap())
        lights_.insert_or_assign(p.first, p.second);

    //create or replace lights specified in settings
    createAndInitializeFromSettings();
}
...
void PawnSimApi::createAndInitializeFromSettings()
{
    USceneComponent* bodyMesh = params_.pawn->GetRootComponent();
    ...
    for (const auto& light_setting_pair : getVehicleSetting()->lights) {
        ...
        //get pose
        FVector position = transform.fromLocalNed(setting.position) - transform.fromLocalNed(Vector3r::Zero());
        FRotator rotation = FRotator(setting.rotation.pitch, setting.rotation.yaw, setting.rotation.roll);
        ...
        if (IsValid(light))
        {
            ...
            //add on to our collection
            lights_.insert_or_assign(light_setting_pair.first, light);
        }      
    }
}
...
// getNedTransform() is inlined in PawnSimApi.h so external modules can use it
// without needing DLL export of a non-virtual accessor.
...
const ALight* PawnSimApi::getLight(const std::string& light_name) const
{
    return lights_.findOrDefault(light_name, nullptr);
}
...
void PawnSimApi::resetImplementation()
{
    ...
    {
        FScopeLock Lock(&external_pose_debug_mutex_);
        external_pose_debug_state_ = ExternalPoseDebugState();
    }
...
void PawnSimApi::reportState(msr::airlib::StateReporter& reporter)
{
    ...
    const ExternalPoseDebugState external_pose = getExternalPoseDebugState();
    reporter.writeHeading("DTAM External Pose");
    reporter.writeValue("DTAM-RX", external_pose.has_received);
    reporter.writeValue("DTAM-RX-Count", external_pose.command_count);
    if (external_pose.has_received) {
        const uint64 now_nanos = msr::airlib::ClockFactory::get()->nowNanos();
        const double age_ms =
            static_cast<double>(now_nanos - external_pose.last_command_timestamp_nanos) / 1.0e6;
        reporter.writeValue("DTAM-RX-AgeMs", age_ms);
        reporter.writeValue("DTAM-IgnoreCollision", external_pose.last_ignore_collision);
        reporter.writeValue("DTAM-NED", external_pose.last_command_pose.position);

        real_T pitch_rad = 0;
        real_T roll_rad = 0;
        real_T yaw_rad = 0;
        VectorMath::toEulerianAngle(external_pose.last_command_pose.orientation, pitch_rad, roll_rad, yaw_rad);
        reporter.writeValue("DTAM-YawDeg", Utils::radiansToDegrees(yaw_rad));
    }
}
...
bool PawnSimApi::setLightVisibility(const std::string& light_name, bool is_visible)
{
...
bool PawnSimApi::setLightIntensity(const std::string& light_name, float intensity)
{
...
void PawnSimApi::setPoseInternal(const Pose& pose, bool ignore_collision)
{
    ...
    if (enable_teleport)
        ...
        FHitResult Hit;
        params_.pawn->SetActorLocationAndRotation(position, orientation, true, &Hit, ETeleportType::None);
        if (Hit.bBlockingHit) {
            // Some externally driven pose updates are swept successfully by
            // Unreal but do not always propagate through the pawn collision
            // delegate path used by AirSim.  Mirror the NotifyHit path here so
            // simGetCollisionInfo() can expose the blocked movement to DTAM
            // 4103 polling.
            onCollision(
                Cast<UPrimitiveComponent>(params_.pawn->GetRootComponent()),
                Hit.GetActor(),
                Hit.GetComponent(),
                true,
                Hit.ImpactPoint,
...
void PawnSimApi::recordExternalPoseCommand(const Pose& pose, bool ignore_collision)
{
    FScopeLock Lock(&external_pose_debug_mutex_);
    external_pose_debug_state_.has_received = true;
    external_pose_debug_state_.last_command_timestamp_nanos = msr::airlib::ClockFactory::get()->nowNanos();
    external_pose_debug_state_.command_count += 1;
    external_pose_debug_state_.last_ignore_collision = ignore_collision;
    external_pose_debug_state_.last_command_pose = pose;
}

PawnSimApi::ExternalPoseDebugState PawnSimApi::getExternalPoseDebugState() const
{
    FScopeLock Lock(&external_pose_debug_mutex_);
    return external_pose_debug_state_;
}
