...
public: //implementation of VehicleSimApiBase
    struct ExternalPoseDebugState
    {
        bool has_received = false;
        uint64 last_command_timestamp_nanos = 0;
        uint64 command_count = 0;
        bool last_ignore_collision = false;
        Pose last_command_pose = Pose::nanPose();
    };
    ...
    void recordExternalPoseCommand(const Pose& pose, bool ignore_collision);
    ExternalPoseDebugState getExternalPoseDebugState() const;
...
public: //Unreal specific methods
    ...
    //return the attached pawn (inlined for cross-module access)
    APawn* getPawn() { return params_.pawn; }
    ...
    // Inlined so external game modules (DTAMVisualization) can use without
    // needing an explicit AIRSIM_API export for this trivial accessor.
    const NedTransform& getNedTransform() const { return ned_transform_; }
...
private: //vars
    ...
    mutable FCriticalSection external_pose_debug_mutex_;
    ExternalPoseDebugState external_pose_debug_state_;
