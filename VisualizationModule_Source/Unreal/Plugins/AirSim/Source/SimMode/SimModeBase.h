...
public:
    ...
    const PawnSimApi* getVehicleSimApi(const APawn* pawn) const;
    PawnSimApi* getVehicleSimApi(APawn* pawn)
    {
        return const_cast<PawnSimApi*>(static_cast<const ASimModeBase*>(this)->getVehicleSimApi(pawn));
    }
    bool setVehiclePose(APawn* pawn, const FVector& position, const FQuat& orientation, bool ignore_collision);
...
private:
    ...
    bool bLoadingScreenVisible_ = false;
