...
{
    bLoadingScreenVisible_ = is_visible;
...
void ASimModeBase::BeginPlay()
{
    ...
    if (have_existing_pawns) {
        ...
        if (APlayerStart* PlayerStart =
                Cast<APlayerStart>(UGameplayStatics::GetActorOfClass(this, APlayerStart::StaticClass()))) {
            fpv_pawn = PlayerStart;
        }
        else {
            APlayerController* player_controller = this->GetWorld()->GetFirstPlayerController();
            fpv_pawn = player_controller ? player_controller->GetViewTarget() : nullptr;
        }
    }

    if (fpv_pawn == nullptr) {
        UE_LOG(LogTemp, Error, TEXT("Could not resolve a startup transform from an existing pawn, PlayerStart, or view target."));
        return;
    }
    ...
    if (loading_screen_widget_ != nullptr) {
        ...
        loading_screen_widget_->SetVisibility(
            bLoadingScreenVisible_ ? ESlateVisibility::Visible : ESlateVisibility::Hidden);
    }
...
const PawnSimApi* ASimModeBase::getVehicleSimApi(const APawn* pawn) const
{
    if (pawn == nullptr) {
        return nullptr;
    }

    for (const auto& sim_api : vehicle_sim_apis_) {
        const PawnSimApi* pawn_sim_api = static_cast<const PawnSimApi*>(sim_api.get());
        if (pawn_sim_api != nullptr && const_cast<PawnSimApi*>(pawn_sim_api)->getPawn() == pawn) {
            return pawn_sim_api;
        }
    }
...
bool ASimModeBase::setVehiclePose(APawn* pawn, const FVector& position, const FQuat& orientation, bool ignore_collision)
{
    PawnSimApi* pawn_sim_api = getVehicleSimApi(pawn);
    if (pawn_sim_api == nullptr) {
        return false;
    }

    const NedTransform& ned_transform = pawn_sim_api->getNedTransform();
    const Pose pose(ned_transform.toLocalNed(position), ned_transform.toNed(orientation));
    pawn_sim_api->recordExternalPoseCommand(pose, ignore_collision);
    pawn_sim_api->setPose(pose, ignore_collision);
    return true;
}
...
void ASimModeBase::setupVehiclesAndCamera()
{
    ...
    if (getApiProvider()->hasDefaultVehicle()) {
        ...
        CameraDirector->initializeForBeginPlay(getInitialViewMode(),
                                               ...
                                               getVehicleSimApi()->getCamera("front_center"),
                                               getVehicleSimApi()->getCamera("back_center"));
    }
    else
