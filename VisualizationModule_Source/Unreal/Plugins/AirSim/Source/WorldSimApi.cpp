...
bool readRenderTargetPixels(UTextureRenderTarget2D* render_target, TArray<FColor>& pixels, int32& width, int32& height)
{
    if (render_target == nullptr) {
        return false;
    }

    FTextureRenderTargetResource* resource = render_target->GameThread_GetRenderTargetResource();
    if (resource == nullptr) {
        return false;
    }
    ...
    pixels.Reset();
    if (!resource->ReadPixels(pixels, flags)) {
        return false;
    }
...
bool encodePixelsToJpeg(const TArray<FColor>& pixels, int32 width, int32 height, int32 quality, std::vector<uint8_t>& encoded)
{
    encoded.clear();
    ...
    if (!image_wrapper->SetRaw(pixels.GetData(), pixels.Num() * sizeof(FColor), width, height, ERGBFormat::BGRA, 8)) {
        return false;
    }

    const auto& compressed = image_wrapper->GetCompressed(FMath::Clamp(quality, 1, 100));
    encoded.assign(compressed.GetData(), compressed.GetData() + compressed.Num());
...
bool WorldSimApi::isWeatherEnabled() const
{
    bool result = false;
    UAirBlueprintLib::RunCommandOnGameThread([this, &result]() {
        result = UWeatherLib::getIsWeatherEnabled(simmode_->GetWorld());
    },
                                             true);
    return result;
}

float WorldSimApi::getWeatherParameter(WeatherParameter param) const
{
    float result = 0.0f;
    unsigned char param_n = static_cast<unsigned char>(msr::airlib::Utils::toNumeric<WeatherParameter>(param));
    EWeatherParamScalar param_e = msr::airlib::Utils::toEnum<EWeatherParamScalar>(param_n);

    UAirBlueprintLib::RunCommandOnGameThread([this, param_e, &result]() {
        result = UWeatherLib::getWeatherParamScalar(simmode_->GetWorld(), param_e);
    },
                                             true);
    return result;
}
...
std::vector<uint8_t> WorldSimApi::getCameraFrame(ImageCaptureBase::ImageType image_type, const CameraDetails& camera_details, const std::string& annotation_name, int quality) const
{
    std::vector<uint8_t> encoded;
    APIPCamera* camera = simmode_->getCamera(camera_details);

    UAirBlueprintLib::RunCommandOnGameThread([camera, image_type, annotation_name, quality, &encoded]() {
        if (camera == nullptr) {
            return;
        }

        if (image_type == ImageCaptureBase::ImageType::Annotation && !camera->GetAnnotationNameExist(annotation_name)) {
            return;
        }

        if (!camera->getCameraTypeEnabled(image_type, annotation_name)) {
            camera->setCameraTypeEnabled(image_type, true, annotation_name);
            camera->setCameraTypeUpdate(image_type, false, annotation_name);
        }

        UTextureRenderTarget2D* render_target = camera->getRenderTarget(image_type, false, annotation_name);
        if (render_target == nullptr) {
            return;
        }
        ...
        int32 width = 0;
        int32 height = 0;
        if (!readRenderTargetPixels(render_target, pixels, width, height)) {
            return;
        }

        encodePixelsToJpeg(pixels, width, height, quality, encoded);
    },
                                             true);
