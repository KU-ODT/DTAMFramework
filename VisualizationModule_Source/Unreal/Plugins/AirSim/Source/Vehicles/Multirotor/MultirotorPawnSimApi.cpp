...
// DTAM: toggled by UDTAMControlModeSubsystem while keyboard/joystick control is active.
// Pending simSetVehiclePose updates are still rendered; only autonomous physics-body
// pose overwrites are suppressed.
static TAutoConsoleVariable<int32> CVarDtamManualFlightActive(
    TEXT("dtam.ManualFlightActive"),
    0,
    TEXT("When non-zero, MultirotorPawnSimApi does not overwrite the pawn pose "
         "from autonomous physics unless a pending simSetVehiclePose update exists."),
    ECVF_Default);
...
void MultirotorPawnSimApi::updateRendering(float dt)
{
    ...
    IConsoleVariable* DtamManualCVar =
        IConsoleManager::Get().FindConsoleVariable(TEXT("dtam.ManualFlightActive"));
    const bool bManualFlightActive = DtamManualCVar && DtamManualCVar->GetInt() != 0;
