...
void APIPCamera::BeginPlay()
{
    ...
    for (unsigned int image_type = 0; image_type < imageTypeCount(); ++image_type) {
        // DTAM: AirSim's Python/HTTP video path reads SceneCapture targets back
        // through PF_B8G8R8A8 PNG/JPEG buffers.  On the UE5 packaged DT World
        // build, SCS_FinalToneCurveHDR can read back as an all-black 8-bit
        // buffer even though the viewport itself is rendering.  Use the standard
        // LDR final-color path for most captures. Scene is overridden to
        // BaseColor below as a DTAM/UE5 fallback because the packaged Cesium
        // lighting path can render all-black in SceneCapture while depth and
        // normals still prove geometry is present.
        ...
        if (Utils::toEnum<ImageType>(image_type) == ImageType::Scene) {
            captures_[image_type]->CaptureSource = ESceneCaptureSource::SCS_BaseColor;
            UE_LOG(LogTemp, Warning, TEXT("DTAM AirSim SceneCapture source set to SCS_BaseColor for visible SituationAwareness RGB feed"));
        }
