...
void RenderRequest::getScreenshot(std::shared_ptr<RenderParams> params[], std::vector<std::shared_ptr<RenderResult>>& results, unsigned int req_size, bool use_safe_method)
{
    ...
    if (use_safe_method) {
        ...
        AsyncTask(ENamedThreads::GameThread, [this]() {
            ...
            // while we're still on GameThread, request a SceneCapture render.
            for (unsigned int i = 0; i < req_size_; ++i) {
                if (params_[i]->render_target != nullptr && params_[i]->render_component != nullptr) {
                    // DTAM/UE5 packaged builds were returning an all-black
                    // RenderTarget for AirSim Scene images because the deferred
                    // capture could execute after our OnEndDraw readback.
                    // Capture immediately on the game thread so the render
                    // target has current pixels before RenderRequest reads it.
                    params_[i]->render_component->CaptureScene();
                }
            }
        });
