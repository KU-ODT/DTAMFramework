"""DTAM Mission Planner service.

This service combines DTAM WebSocket communication with mission ICD build
helpers. It handles inbound 2001/2002 messages, automatically builds and sends
3001 Scheduled Flight messages, and exposes helper methods for route APIs.
"""
...
_SDK_ROOT = Path(__file__).resolve().parents[3] / "DTAMSDK"
...
def _normalize_std_value(value: Any, fallback: Optional[str] = None) -> str:
    ...
    if fallback:
        return _normalize_std_value(fallback, None)
    return datetime.datetime.now().strftime("%H:%M:%S")


def _departure_std_from_entry(entry: Dict[str, Any], fallback: str) -> str:
    options = entry.get("options") if isinstance(entry.get("options"), dict) else {}
    for value in (
        options.get("std"),
        options.get("departureTime"),
        options.get("departure_time"),
        entry.get("std"),
        entry.get("departureTime"),
        entry.get("departure_time"),
    ):
        if str(value or "").strip():
            return _normalize_std_value(value, fallback)
    return _normalize_std_value(fallback)


def _ensure_plan_metadata(record: Dict[str, Any]) -> Dict[str, Any]:
    """Fill ``planVersion`` and ``planStatus`` for the 3001 schema."""
    out = deepcopy(record)
...
class MissionService(MissionModule):
    """Mission Planner DTAM communication and ICD build facade."""
    ...
    def on_flight_plan_request(self, msg: Any) -> None:
        """Handle MSG 2001 and trigger automatic 3001 generation.

        Flow: counters -> scenario lookup -> mission payload -> ICD bundle -> send_scheduled_flights -> status update.
        """
        raw = _result_raw(msg)
    ...
    # Receive handlers
    # ICD bundle helpers (called by routes)
    # Receive handlers
    @staticmethod
    ...
    def build_existing_icd_export_bundle(
        ...
        normalized_records = [_ensure_plan_metadata(item) for item in records if isinstance(item, dict)]
    ...
    def build_mission_icd_bundle(
        ...
        for mission_payload, fleet_entry in zip(missions, normalized_fleet):
            ...
            item_options["std"] = _departure_std_from_entry(item_payload, str(item_options.get("std") or ""))
            item_options["aircraftId"] = fleet_entry["aircraftId"]
            item_options["flightPlanNumber"] = fleet_entry["flightPlanNumber"]
            item_payload["options"] = item_options
            exports.append(
    ...
    def _build_mission_payload_from_sim_mode(
        ...
        std = _normalize_std_value(operation_time.get("startTime"), datetime.datetime.now().strftime("%H:%M:%S"))
        base_number = int(datetime.datetime.now().strftime("%m%d%H%M"))
        ...
        for index, entry in enumerate(valid_entries):
            ...
            entry_std = _departure_std_from_entry(entry, std)
            fleet.append({
                "aircraftId": aircraft_id,
