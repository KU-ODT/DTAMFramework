...
def validate_mission_icd_record(record: Dict[str, Any]) -> List[str]:
    ...
    for key in ("flightPlanNumber", "planVersion", "planStatus", "aircraftId", "departure", "enRoute", "arrival"):
        if key not in record:
            errors.append(f"Missing top-level field: {key}")
    plan_version = _coerce_int(record.get("planVersion"))
    if plan_version is None or plan_version < 1:
        errors.append("planVersion must be an integer greater than or equal to 1.")
    plan_status = str(record.get("planStatus") or "").strip().lower()
    if plan_status not in PLAN_STATUSES:
        errors.append("planStatus must be one of active, superseded, discarded.")
...
def _build_export_context(options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    plan_version = _coerce_int(options.get("planVersion"))
    if plan_version is None or plan_version < 1:
        plan_version = 1
    plan_status = str(options.get("planStatus") or "active").strip().lower()
    if plan_status not in PLAN_STATUSES:
        plan_status = "active"
    return {
...
def _build_route_record(
    ...
    dep_gate = _pick_unreal_resource_point(departure_name, "GATE") or _pick_resource_point(resources, departure_name, "GATE")
    dep_fato = _pick_unreal_resource_point(departure_name, "FATO", preferred_label="FATO 2") or _pick_resource_point(resources, departure_name, "FATO", preferred_label="FATO 2")
    arr_gate = _pick_unreal_resource_point(arrival_name, "GATE") or _pick_resource_point(resources, arrival_name, "GATE")
    arr_fato = _pick_unreal_resource_point(arrival_name, "FATO", preferred_label="FATO 1") or _pick_resource_point(resources, arrival_name, "FATO", preferred_label="FATO 1")
    ...
    dep_gate_lla, dep_fato_lla = _apply_departure_surface_altitudes(route_data, dep_gate_lla, dep_fato_lla)
    ...
    arrival_touchdown = _apply_arrival_surface_altitude(route_data, arrival_touchdown)
    ...
    phase_k_end = _clamp_phase_k_end(
        current,
...
def _load_resource_catalog(path: Path) -> Dict[str, Dict[str, List[ResourcePoint]]]:
    ...
    for row in rows:
        ...
        yaw_deg = _coerce_float(row.get("Yaw_deg"))
...
def _pick_unreal_resource_point(
    vertiport_name: str,
    category: str,
    *,
    ...
    spawn = lookup_unreal_spawn_point(
        vertiport_name,
        category=category,
    ...
    if spawn is None:
        return None
    lat = _coerce_float(spawn.get("lat"))
    lon = _coerce_float(spawn.get("lon"))
    if lat is None or lon is None:
        return None
    terrain_h = _coerce_float(spawn.get("terrain_h_m"))
    deck_h = _coerce_float(spawn.get("deck_height_m"))
    alt_m = _coerce_float(spawn.get("alt_m"))
    if terrain_h is not None:
        alt_m = float(terrain_h) + float(deck_h or 0.0)
    label = str(spawn.get("spawn_label") or spawn.get("label") or preferred_label or category).strip()
    return ResourcePoint(
        label=label,
        category=str(spawn.get("category") or category).strip().upper(),
        lat=float(lat),
        lon=float(lon),
        alt_m=float(alt_m or 0.0),
        yaw_deg=_coerce_float(spawn.get("yaw_deg")),
    )
...
def _resource_or_port_lla(
    ...
    if resource is not None:
        result = _lla(resource.lat, resource.lon, resource.alt_m)
        if resource.yaw_deg is not None:
            result["yaw_deg"] = float(resource.yaw_deg) % 360.0
        return result
    warnings.append(f"Using {port.name} center as fallback for {label}.")
    return _lla(port.lat, port.lon, fallback_ground_m)


def _copy_lla_with_alt(point: Dict[str, float], alt_m: float) -> Dict[str, float]:
    result = _lla(point["lat"], point["lon"], alt_m)
    for key in ("heading_deg", "yaw_deg"):
        value = point.get(key)
...
def _surface_clearance_alt(source: Optional[Dict[str, Any]], fallback_alt_m: float) -> float:
    if isinstance(source, dict):
        ground_m = _coerce_float(source.get("ground_m"))
        if ground_m is not None:
            return ground_m + GROUND_CLEARANCE_M
        alt_m = _coerce_float(source.get("alt_m"))
        if alt_m is not None:
            return alt_m
    return float(fallback_alt_m)


def _route_point_at(route_data: Dict[str, Any], index: int) -> Optional[Dict[str, Any]]:
    for key in ("points", "missionWaypoints", "waypoints"):
...
def _apply_departure_surface_altitudes(
    route_data: Dict[str, Any],
    departure_gate_lla: Dict[str, float],
    departure_takeoff_lla: Dict[str, float],
) -> tuple[Dict[str, float], Dict[str, float]]:
    start_source = _route_point_at(route_data, 0)
    takeoff_source = route_data.get("departureTakeoff")
    end_alt_m = _surface_clearance_alt(takeoff_source, departure_takeoff_lla["alt"])
    start_alt_m = _surface_clearance_alt(start_source, departure_gate_lla["alt"])
    if start_alt_m > end_alt_m:
        start_alt_m = end_alt_m - PHASE_ENDPOINT_MARGIN_M
    return (
        _copy_lla_with_alt(departure_gate_lla, start_alt_m),
        _copy_lla_with_alt(departure_takeoff_lla, end_alt_m),
    )


def _apply_arrival_surface_altitude(
    route_data: Dict[str, Any],
    arrival_landing_lla: Dict[str, float],
) -> Dict[str, float]:
    landing_source = route_data.get("arrivalTouchdown")
    if not isinstance(landing_source, dict):
        landing_source = _route_point_at(route_data, -1)
    return _copy_lla_with_alt(
        arrival_landing_lla,
        _surface_clearance_alt(landing_source, arrival_landing_lla["alt"]),
    )


def _clamp_phase_k_end(
    phase_k_start: Dict[str, float],
    phase_k_end: Dict[str, float],
) -> Dict[str, float]:
    start_alt_m = float(phase_k_start["alt"])
    end_alt_m = float(phase_k_end["alt"])
    if end_alt_m > start_alt_m:
        return _copy_lla_with_alt(phase_k_end, start_alt_m - PHASE_ENDPOINT_MARGIN_M)
    return phase_k_end
...
def _spawn_lla_for_port(
    *,
    ...
    if unreal_spawn is not None:
        lat = _coerce_float(unreal_spawn.get("lat"))
        lon = _coerce_float(unreal_spawn.get("lon"))
        if lat is not None and lon is not None:
            ground_m = _coerce_float(unreal_spawn.get("ground_m"))
            alt_m = _coerce_float(unreal_spawn.get("alt_m"))
            if alt_m is None:
                alt_m = float(ground_m if ground_m is not None else fallback_ground_m) + GROUND_CLEARANCE_M
            result = _lla(float(lat), float(lon), float(alt_m))
            frame_yaw = _frame_yaw_from_waypoint(unreal_spawn)
            heading = _heading_from_waypoint(unreal_spawn)
    ...
    result = _lla(
        float(spawn["lat"]),
        float(spawn["lon"]),
        float(spawn.get("ground_m") or fallback_ground_m) + GROUND_CLEARANCE_M,
    )
