  ...
  function getExecutionWaypoints() {
    if (mode === 'route' && routeData) {
      if (Array.isArray(routeData.points) && routeData.points.length > 1) {
        const departureTakeoffAltM = Number(routeData?.departureTakeoff?.alt_m ?? routeData?.departureTakeoff?.ground_m);
        ...
        return routeData.points.map((point, index) => {
          ...
          if (index === 0 && Number.isFinite(departureTakeoffAltM)) {
            altM = departureTakeoffAltM;
          }
