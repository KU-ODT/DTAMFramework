(() => {
  ...
  class MapApp {
    ...
    async loadResourcesVp() {
      ...
      try {
        const resourceUrlCandidates = [
          "/data/unreal_vertiport_spawn_points.csv",
          "/data/resources_vp.csv",
        ];
        let response = null;
        for (const url of resourceUrlCandidates) {
        ...
        if (!response) {
          throw new Error("No vertiport resource CSV is available.");
        }
        const text = await response.text();
        const rows = normalizeRows(parseCsvRows(text));
        if (!rows.length) {
        ...
        const idxZ = firstHeaderIndex("route_ground_m", "deck_h_m", "z_m", "resource_z_m");
        const idxXm = firstHeaderIndex("airsim_x_m", "x_m", "resource_x_m");
        const idxYm = firstHeaderIndex("airsim_y_m", "y_m", "resource_y_m");
        const idxXcm = header.indexOf("x_cm");
        const idxYcm = header.indexOf("y_cm");
        const idxZcm = header.indexOf("z_cm");
        const idxLat = firstHeaderIndex("lla_lat_deg", "pt_lat_deg", "resource_lat_deg");
        const idxLon = firstHeaderIndex("lla_lon_deg", "pt_lon_deg", "resource_lon_deg");
        if (idxVertiport < 0 || idxLabel < 0 || idxZ < 0) {
          return;
        }
        const altLookup = new Map();
        const resourceLookup = new Map();
        const vertiportAliases = new Map([
          ["영등포", "여의도"],
          ["여의도", "영등포"],
        ]);
        ...
        rows.slice(1).forEach((row) => {
          ...
          if (Number.isFinite(z)) {
            altLookup.set(vp, z);
            const alias = vertiportAliases.get(vp);
            if (alias) {
              altLookup.set(alias, z);
            }
          }
          ...
          if (Number.isFinite(lat) && Number.isFinite(lon)) {
            ...
            resourceLookup.set(vp, resourceEntry);
            const alias = vertiportAliases.get(vp);
            if (alias) {
              resourceLookup.set(alias, resourceEntry);
            }
          }
        });
        if (altLookup.size) {
        ...
        console.warn("Failed to load vertiport resource CSV.", error);
      }
    }
